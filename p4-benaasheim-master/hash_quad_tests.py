...
class TestList(unittest.TestCase):
    ...
    def test_04(self):
        ...
        self.assertEqual(ht.get_index("mouse"), 13)
        self.assertEqual(ht.get_index("elephant"), 9)
        self.assertEqual(ht.get_index("gorillass"), 6)
        keys = ht.get_all_keys()
        keys.sort()
        self.assertEqual(keys, ["cat", "dog", "elephant", "gorillass", "mouse"])
        
        ht.insert("gorillasd", 7)
        self.assertEqual(ht.get_num_items(), 6)
        self.assertEqual(ht.get_table_size(), 15)
        self.assertAlmostEqual(ht.get_load_factor(), 6 / 15)
        self.assertEqual(ht.get_index("cat"), 12)
        self.assertEqual(ht.get_index("dog"), 14)
        self.assertEqual(ht.get_index("dg"), None)
        self.assertEqual(ht.get_value("dg"), None)
        self.assertEqual(ht.get_index("mouse"), 13)
        self.assertEqual(ht.get_index("elephant"), 9)
        self.assertEqual(ht.get_index("gorillass"), 6)
        keys = ht.get_all_keys()
        keys.sort()
        self.assertEqual(keys, ["cat", "dog", "elephant", "gorillasd", "gorillass", "mouse"])
